...
$out = Join-Path ([Environment]::GetFolderPath("Desktop")) "RAPOR_MUHUR_v3025.txt"
"=== v3.0.25 MUHUR RAPORU (HybridTool) - $($(Get-Date -Format 'yyyy-MM-dd HH:mm')) ===" | Out-File $out -Encoding utf8
function B($t,$s){ "
...
B "5. KIMLIKLI TEST GOVDELERI (T260-T269 HybridTool)" {
  Get-Content tests/TrainService.Cad.Tests/Tools/T260_HybridToolTests.cs -EA SilentlyContinue }

B "6. HybridTool KAYNAK KODU" {
  Get-Content src/TrainService.Cad/Tools/HybridTool.cs -EA SilentlyContinue }

B "7. PreviewHybrid record (ITool.cs)" {
  Get-Content src/TrainService.Cad/Tools/ITool.cs -EA SilentlyContinue | Select-String "PreviewHybrid" -Context 20,0 }

B "8. TEST SAYILARI (proje bazinda)" {
  dotnet test TrainService.sln -c Release --nologo | Select-String "Passed|Failed|Total tests" }

B "9. SAPMA BEYANI (v3.0.25)" {
  Get-Content tools/sapma.txt -EA SilentlyContinue | Select-String "v3.0.25" -Context 0,20 }
